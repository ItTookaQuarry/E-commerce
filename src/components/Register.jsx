...
export async function action({ request }) {
 ...
 try {
  ...
  await addDoc(users,{email:email,})
...
const data=useActionData()
  ...
  return (
      ...
      <Form className="loginchild" method="post">
        ...
        <input name="secondpassword" type="password" placeholder="password" className="imput"/>
        <br></br>
        <button type="submit" >Create Account</button>
      </Form>
      {data?.email&&<p style={{color:"black"}} className="p">{`new account  created`}</p>}
